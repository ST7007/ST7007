...
// Delete an enquiry
app.delete("/api/enquiry/delete/:id",(req,res)=>{
...
// Update an enquiry
app.put("/api/enquiry/update/:id",(req,res)=>{

  const id=req.params.id;
  const {name,phone,pickup,dropoff,date,status}=req.body;

  db.run(
    `UPDATE enquiries 
     SET name=?, phone=?, pickup=?, dropoff=?, date=?, status=? 
     WHERE id=?`,
    [name,phone,pickup,dropoff,date,status,id],
    function(err){
...
app.get("/admin/enquiries", (req, res) => {
  res.sendFile(__dirname + "/admin/enquiries.html");
});
